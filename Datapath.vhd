...
architecture structural of Datapath is
	...
	-- Instruction word from ProgMem (goes directly to IR, not through BUS)
	...
	-- Sign-extended immediates (for BUS driving)
...
begin
	...
	-----------------------------------------------------------------
	-- Program Memory (ITCM): unregistered (combinational) read
	-- Read address = lower AddrWidth bits of PC
	-- Write port used only by testbench to load the program
	-----------------------------------------------------------------
	ITCM_inst : ProgMem
	...
	-----------------------------------------------------------------
	-- Register File: 16 × 16-bit general-purpose registers
	-- R[0] is hard-wired to zero (enforced in RF.vhd)
	-----------------------------------------------------------------
	RF_inst : RF
	...
	-----------------------------------------------------------------
	-- DTCM address register: latches lower AddrWidth bits of BUS
	-- as the memory address for subsequent DTCM read/write
	-----------------------------------------------------------------
	process(clk, rst)
	begin
		if rst = '1' then
	...
	-----------------------------------------------------------------
	-- DTCM mux: testbench takes over addresses/data when TBactive='1'
	-----------------------------------------------------------------
	...
	-----------------------------------------------------------------
	-- Data Memory (DTCM): registered (clocked) read output
	-----------------------------------------------------------------
	DTCM_inst : dataMem
		generic map(Dwidth => BusWidth, Awidth => AddrWidth, dept => 2**AddrWidth)
		port map(
